...
JumpTable jumpTable = {
  ...
  {Opcode::EXTENSION, new ExtensionOperation()},
...
ExtensionJumpTable extensionJumpTable = {};
...
ExecStatus ExtensionOperation::execute(CallContext& context) {
  // TODO: Check code length
  context.incPc();
  ExtensionOpcode extOpcode = context.getContract()->getExtensionOpcodeAt(context.getPc());

  Operation* operation = getExtensionOperation(extOpcode);
...
Operation* getExtensionOperation(ExtensionOpcode opcode) {
  return extensionJumpTable[opcode];
}
